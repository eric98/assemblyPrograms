...
; ASCII / ATTR CODES TO DRAW THE SNAKE
    ASCII_SNAKE     EQU 02Ah ; caracter
    ATTR_SNAKE      EQU 070h ; color_fondo

; ASCII / ATTR CODES TO DRAW THE FIELD
    ASCII_FIELD    EQU 020h ; caracter
    ATTR_FIELD     EQU 070h ; color_fondo
...
NEW_TIMER_INTERRUPT PROC NEAR
    ...
    MOV AL, [INT_COUNT]
    CMP [DIV_SPEED], AL
    JNZ END_ISR 		; if (int_count != div_speed) end_isr()
    MOV [INT_COUNT], 0	; else int_count = 0
	
	; borrar ultima posicion
	;MOV AL, 02Ah ; caracter
    ;MOV BL, 14h ; color_fondo
    ...
    INC [NUM_TILES] ; = ADD [NUM_TILES], 1
    CALL PRINT_SNAKE

    ; Check if it is time to increase the speed of the snake
    CMP [DIV_SPEED], 1
    JZ END_ISR ; if (div_speed == 1) end_isr();
    MOV AX, [NUM_TILES]
    DIV [NUM_TILES_INC_SPEED]	; AH = NUM_TILES/NUM_TILES_INC_SPEED
    CMP AH, 0                 ; REMAINDER
    JNZ END_ISR				  ; (NUM_TILES/NUM_TILES_INC_SPEED) != 0
    DEC [DIV_SPEED]
...
DATA_SEG	SEGMENT	PUBLIC
    ...
    DIV_SPEED DB 1             ; THE SNAKE SPEED IS THE (INTERRUPT FREQUENCY) / DIV_SPEED
